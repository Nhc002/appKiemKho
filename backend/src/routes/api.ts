...
router.get('/dashboard', async (req: Request, res: Response) => {
  try {
    ...
    const ingredientLossMap: Record<number, { name: string; quantity: number; unit: string; cost: number }> = {};
    counts.forEach(c => {
      c.items?.forEach(it => {
        if (it.difference < 0) {
...
router.get('/recipes/:productId', async (req: Request, res: Response) => {
  try {
...
router.post('/recipes', async (req: Request, res: Response) => {
  try {
    ...
    await LogRepository.add(
      ...
      `Cấu hình công thức (BOM) cho món: ${prod ? prod.item_name : `#${product_id}`} với ${items.length} nguyên vật liệu.`
    );
...
router.post('/import', async (req: Request, res: Response) => {
  try {
    ...
    const newImport = await InventoryRepository.addImport(
      { supplier_id, total_cost, note, user_id },
      items
    );
    res.json(newImport);
  } catch (err: any) {
...
router.post('/export', async (req: Request, res: Response) => {
  try {
    ...
    const newExport = await InventoryRepository.addExport(
      { note, user_id, total_value },
      items
    );
    res.json(newExport);
  } catch (err: any) {
...
router.post('/inventory-count', async (req: Request, res: Response) => {
  try {
    const { user_id, total_difference_cost, note, items } = req.body;
    const newCount = await InventoryRepository.addCount(
      { user_id, total_difference_cost, note },
      items
    );
    res.json(newCount);
  } catch (err: any) {
...
// ==========================================
router.post('/fabi/sync', async (req: Request, res: Response) => {
  try {
