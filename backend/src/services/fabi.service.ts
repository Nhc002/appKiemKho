...
export class FabiService {
   ...
   */
  static async syncSales(startDateStr?: string, endDateStr?: string): Promise<{
    sync_id: number;
    ...
    for (const rawItem of itemsReport) {
      ...
      if (!product) {
        ...
        await LogRepository.add(
          'Đồng bộ Fabi', 
          `Phát hiện món mới từ iPOS: ${item_name} (${item_id}). Đã tạo sản phẩm và đánh dấu 'Recipe Missing' để cấu hình định lượng sau.`
        );
      } else {
    ...
    await LogRepository.add(
      'Đồng bộ doanh số', 
      `Đồng bộ thành công dữ liệu bán hàng iPOS (${start} -> ${end}). Doanh thu Net: ${totalRevenueNet.toLocaleString('vi-VN')}₫, Giá vốn COGS: ${totalCogs.toLocaleString('vi-VN')}₫. Trừ nguyên liệu kho thành công.`
    );
   ...
   */
  private static generateMockFabiSales(): any[] {
